...
import { place3DBet, get3DStatus } from '../../services/api';
...
export const ThreeDModal: React.FC<ThreeDModalProps> = ({ isOpen, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validate numbers
    const invalidNumbers = numbers.some(num => num.length !== 3);
    if (invalidNumbers) {
      setError('All numbers must be 3 digits');
      return;
    }
    ...
    try {
      await place3DBet(numbers, numericAmounts);
      onSuccess?.();
      onClose();
    } catch (err) {
      const error = err as { response?: { data: ErrorResponse } };
